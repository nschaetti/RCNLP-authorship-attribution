...
model = torchlanguage.models.CCSAA(
    ...
    n_classes=50 - settings.n_authors,
    n_features=50
)
if args.cuda:
    model.cuda()
...
for epoch in range(settings.ccsaa_epoch):
    ...
    for i, data in enumerate(reuters_loader_train):
        ...
        # Run training
        if i % settings.ccsaa_files == settings.ccsaa_files - 1:
            # Append
            ...
            # For each batch
            for batch_i in np.arange(0, samples_inputs.size(0), settings.ccsaa_batch_size):
                # Batch samples
                batch_inputs = samples_inputs[int(batch_i):int(batch_i) + settings.ccsaa_batch_size]
                batch_outputs = samples_outputs[int(batch_i):int(batch_i) + settings.ccsaa_batch_size]
    ...
    # Print and save loss
    print(
        u"Epoch {}, training files {}, training loss {}, test files {}, test loss {}, accuracy {}, doc accuracy {}".format(
            epoch,
            training_files,
    ...
    if accuracy > best_acc:
        plt.imsave("ccsaa35_outputs.{}.png".format(epoch), last_embeddings.data.t().cpu().numpy(), cmap='Greys')
        best_acc = accuracy
        # Save model
        print(u"Saving model with best accuracy {}".format(best_acc))
        torch.save(model.state_dict(), open(
            os.path.join(args.output, u"ccsaa35.pth"),
            'wb'))
        torch.save(transform.transforms[1].token_to_ix, open(
            os.path.join(args.output, u"ccsaa35.voc.pth"),
            'wb'))
    # end if
# end for
