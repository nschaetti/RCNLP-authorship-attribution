...
class CNN2DDeepFeatureSelector(nn.Module):
    ...
    # Constructor
    def __init__(self, n_gram, n_authors=15, embedding_dim=300, out_channels=(20, 10), kernel_sizes=(5, 5), n_features=30):
        """
        Constructor
        :param n_authors:
        ...
        # Max pooling layer
        self.max_pool = nn.MaxPool2d(kernel_size=embedding_dim - out_channels[0] + 1, stride=0)

        # Conv 2
        self.conv2 = nn.Conv1d(in_channels=out_channels[0], out_channels=out_channels[1], kernel_size=(n_gram, kernel_sizes[1]))

        # Linear layer 1
        self.linear_size = out_channels[1] * n_gram * kernel_sizes[1]
    ...
    def forward(self, x):
        ...
        # Max pooling
        max_pooled = self.max_pool_w1(out_conv1)

        # Conv 2
        out_conv2 = F.relu(self.conv2(max_pooled))

        # Max pooling
        max_pooled = self.max_pool_w1(out_conv2)
