...
model = torchlanguage.models.CGFS(
    n_gram=args.n_gram,
    n_authors=50 - settings.n_authors,
...
# Forward hook
model.linear3.register_forward_hook(get_embeddings)
...
for epoch in range(settings.cgfs_epoch):
    ...
    for i, data in enumerate(reuters_loader_train):
        ...
        # Run training
        if i % settings.cgfs_files == settings.cgfs_files - 1:
            # Append
            ...
            # For each batch
            for batch_i in np.arange(0, samples_inputs.size(0), settings.cgfs_batch_size):
                # Batch samples
                batch_inputs = samples_inputs[int(batch_i):int(batch_i)+settings.cgfs_batch_size]
                batch_outputs = samples_outputs[int(batch_i):int(batch_i)+settings.cgfs_batch_size]
    ...
    # Print and save loss
    print(u"Epoch {}, training files {}, training loss {}, test files {}, test loss {}, accuracy {}, doc accuracy {}".format(
        epoch,
        training_files,
    ...
    if accuracy > best_acc:
        plt.imsave("cgfs_outputs.{}.png".format(epoch), last_embeddings.data.t().cpu().numpy(), cmap='Greys')
        plt.show()
        best_acc = accuracy
        # Save model
        print(u"Saving model with best accuracy {}".format(best_acc))
        torch.save(
            model.state_dict(), open(os.path.join(args.output, u"cgfs35.p"), 'wb')
        )
    # end if
# end for