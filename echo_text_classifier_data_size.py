...
args.add_argument(command="--dataset-size", name="dataset_size", type=int,
                  help="Number of text per author (up to 100)", extended=False, default=100)
args.add_argument(command="--dataset-start", name="dataset_start", type=float,
                  help="Where to start in the data set", extended=True, default=100.0)
...
k = args.k if args.k <= args.dataset_size else args.dataset_size
...
# Iterate
for index, space in enumerate(param_space):
    # Params
    ...
    # Corpus
    reteursC50 = Corpus(args.dataset, n_authors=args.n_authors, dataset_size=args.dataset_size, dataset_start=dataset_start)
    author_list = reteursC50.get_authors_list()

    # Author list
    authors = reteursC50.get_authors()
    if index == 0:
